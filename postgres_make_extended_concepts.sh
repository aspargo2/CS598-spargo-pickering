...
#{ echo "${PSQL_PREAMBLE}; DROP TABLE IF EXISTS colloid_bolus; CREATE TABLE colloid_bolus AS "; cat $MIMIC_CODE_DIR/concepts/fluid_balance/colloid_bolus.sql; } | gsed -r -e "${REGEX_DATETIME_DIFF}" | gsed -r -e "${REGEX_SCHEMA}" | psql ${CONNSTR}
#{ echo "${PSQL_PREAMBLE}; DROP TABLE IF EXISTS crystalloid_bolus; CREATE TABLE crystalloid_bolus AS "; cat $MIMIC_CODE_DIR/concepts/fluid_balance/crystalloid_bolus.sql; } | gsed -r -e "${REGEX_DATETIME_DIFF}" | gsed -r -e "${REGEX_SCHEMA}" | psql ${CONNSTR}

{ echo "${PSQL_PREAMBLE}; DROP TABLE IF EXISTS colloid_bolus; CREATE TABLE colloid_bolus AS cob"; cat $MIMIC_CODE_DIR/concepts/fluid_balance/colloid_bolus.sql; } | gsed -r -e "${REGEX_DATETIME_DIFF}" | gsed -r -e "${REGEX_SCHEMA}" | psql ${CONNSTR}
{ echo "${PSQL_PREAMBLE}; DROP TABLE IF EXISTS crystalloid_bolus; CREATE TABLE crystalloid_bolus AS crb"; cat $MIMIC_CODE_DIR/concepts/fluid_balance/crystalloid_bolus.sql; } | gsed -r -e "${REGEX_DATETIME_DIFF}" | gsed -r -e "${REGEX_SCHEMA}" | psql ${CONNSTR}
